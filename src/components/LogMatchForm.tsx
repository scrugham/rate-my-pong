...
const PILL_HOLD_REDUCED_MS = 350;
...
export function LogMatchForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const hold = reduceMotion ? PILL_HOLD_REDUCED_MS : PILL_HOLD_MS;
  ...
  const resultSub =
    games.length > 1
      ? `Series logged · ${games.length} games`
      : `${format} · logged`;
  ...
  return (
    <>
      ...
      {resultPills && (
        <div
          className="log-result-overlay"
          data-leaving={resultLeaving ? "true" : undefined}
          role="status"
          aria-live="polite"
        >
          <p className="log-result-title">Logged</p>
          <p className="log-result-sub">{resultSub}</p>
          <div className="log-result-pills">
            {resultPills.map((p, i) => (
              <span
                key={p.id}
                className="log-result-pill"
                data-dir={p.delta >= 0 ? "up" : "down"}
          ...
          <p className="log-result-go">Opening leaderboard…</p>
        </div>
      )}
    </>
  );
